...
bright gray, doesn't exist as a one number colorcode in ANSI/VT100/xterm and for that I used the remaining colorcode: bright green. 
...
the current foreground and background colors, but in Python82 they first change the current colors to black on bright white. "\n" and "\b" work as expected,
but "\t" only types two spaces and doesn't jump to tab stops. "\r" is ignored, but should have moved the cursor to the beginning of the current line.
Do you know what terminal standard TI follows?
...
One script, FlappBat (i.e. Flappy Bat), is an arcade game where you control the height of a bat using Enter/Entrer several times to eat moths. 
